...
- **Section 6 (MyBO vs open source):** Each function notebook compares **MyBO** (this notebook’s `next_x`) with **Optuna-TPE** (multivariate TPE), **Optuna-GP** (Optuna `GPSampler` via `optuna_solver.suggest(..., sampler="gp")`), **TuRBO**, and **GA**. Wrappers live in `src/optimizers/wrappers/`; **`NEXT_QUERY_SOLUTION`** selects which suggestion is exported to `data/submissions/function_N/next_input_portal.txt` (run §6 before exporting a non-MyBO solver). Explicit arguments to `optuna_solver.suggest` override `configs/optuna_optimizer.yaml` so TPE and GP are not both forced to the same sampler.
- **`IF_EXPORT_PLOT`:** Defaults to **False** in notebooks; set **True** to write figures (e.g. under `data/results/`).
- **Docs:** `README.md` (overview, workflow, structure), `docs/project_roadmap.md` (structure and roadmap), `docs/TECHNICAL_FOUNDATIONS.md` (justification and references). **Scripts:** `run_all.py` runs any `scripts/*.py` (e.g. `append_week1_results.py` … `append_week7_results.py`) then prints submission strings.
