...
- **Acquisition:** All function notebooks use **skopt** (`gaussian_ei`, `gaussian_pi`, `gaussian_lcb`). The default acquisition is configurable via `SOLO_STRATEGY` (EI, PI, or UCB); typically EI. To avoid re-evaluating at or near existing points, candidates within `MIN_DIST_THRESHOLD` (default 0.05) of any observation are masked out. Optionally, candidates near domain boundaries are masked when `BOUNDARY_MARGIN` > 0 (F1–F3 use 0.05; F4–F8 use 0 for high dimensions). The acquisition cell defines `next_x_high_dist` and uses a fallback so `BOUNDARY_MARGIN` defaults to 0 if the parameters cell was not run. If the acquisition argmax would fall in the masked set, the next query uses the high-distance fallback. Alternatives: UCB, high-distance, exploit, documented in each notebook.
- **Ensemble:** `docs_private/20_notebooks_for_devel/function_0_devel.ipynb` (if present) illustrates ensemble acquisition (EI+PI+UCB): centroid when they disagree, EI when they agree. See `docs_private/40_notes_and_references/00_bayesian_methods/ensemble_acquisition_guide.md`.
- **MyBO YAML:** Optional `configs/bayesian_optimizer.yaml` with `default` and `function_1`…`function_8` sections is merged by `my_gp_skopt.load_mybo_config()` into `suggest()`. If the file does not exist, the loader returns an empty dict and **code defaults** apply (see README).
- **Section 6 (MyBO vs open source):** Each function notebook compares **MyBO** (this notebook’s `next_x`) with **Optuna-TPE** (multivariate TPE), **Optuna-GP** (Optuna `GPSampler` via `optuna_solver.suggest(..., sampler="gp")`), **TuRBO**, and **DE-GP-EI** (differential evolution on GP Expected Improvement; code in `de_gp_ei_solver.py`). Wrappers live in `src/optimizers/wrappers/`; **`NEXT_QUERY_SOLUTION`** selects which suggestion is exported to `data/submissions/function_N/next_input_portal.txt` (run Section 6 before exporting a non-MyBO solver; legacy alias `GA` → DE-GP-EI). Explicit arguments to `optuna_solver.suggest` override `configs/optuna_optimizer.yaml` so TPE and GP are not both forced to the same sampler.
- **`IF_EXPORT_PLOT`:** Defaults to **False** in notebooks; set **True** to write figures under `data/results/function_N/` (same `N` as the notebook).
- **Docs:** `README.md` (overview, workflow, structure), `docs/project_roadmap.md` (structure and roadmap), `docs/TECHNICAL_FOUNDATIONS.md` (justification and references). **Automation:** `run_pipeline.py` runs any `append_results/*.py` (including `append_week{N}_results.py` after each challenge round) then prints submission strings.
