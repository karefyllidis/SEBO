...
- **Ensemble:** `docs_private/notebooks/function_0_devel.ipynb` implements ensemble acquisition (EI+PI+UCB): centroid when they disagree, EI when they agree. See `docs_private/40_notes_and_references/ensemble_acquisition_guide.md`.
- **Section 6 (MyBO vs open source):** Each function notebook compares **MyBO** (this notebook’s `next_x`) with **Optuna-TPE** (multivariate TPE), **Optuna-GP** (Optuna `GPSampler` via `optuna_solver.suggest(..., sampler="gp")`), **TuRBO**, and **DE-GP-EI** (differential evolution on GP Expected Improvement; code in `ga_solver.py`). Wrappers live in `src/optimizers/wrappers/`; **`NEXT_QUERY_SOLUTION`** selects which suggestion is exported to `data/submissions/function_N/next_input_portal.txt` (run §6 before exporting a non-MyBO solver; legacy alias `GA` → DE-GP-EI). Explicit arguments to `optuna_solver.suggest` override `configs/optuna_optimizer.yaml` so TPE and GP are not both forced to the same sampler.
- **`IF_EXPORT_PLOT`:** Defaults to **False** in notebooks; set **True** to write figures (e.g. under `data/results/`).
- **Docs:** `README.md` (overview, workflow, structure), `docs/project_roadmap.md` (structure and roadmap), `docs/TECHNICAL_FOUNDATIONS.md` (justification and references). **Scripts:** `run_all.py` runs any `scripts/*.py` (including `append_week{N}_results.py` after each challenge round) then prints submission strings.
