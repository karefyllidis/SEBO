...
- **Committed from `docs_private/`:** `README_PRIVATE.md`, this file, and `20_notebooks_for_devel/` only (see root `.gitignore`). Everything else under `docs_private/` (e.g. `40_notes_and_references/`) stays **local only** — do not push oracle trails or private coursework drafts to a public remote.
