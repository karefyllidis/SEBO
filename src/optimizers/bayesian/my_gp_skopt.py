...
-------------------------
Pass config_path and/or function_id (1..8) to load per-function settings from
configs/bayesian_optimizer.yaml. Each function can override default and function-specific
keys (output_warping, xi, min_dist_threshold, boundary_margin, n_cand, etc.).
Explicit kwargs to suggest() override the config. See load_mybo_config().
"""
...
def load_mybo_config(
    ...
    project_root : str or Path or None
        Project root. If None, inferred from this file's location (two levels up from src/optimizers/bayesian/).
